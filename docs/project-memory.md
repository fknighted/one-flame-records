...
**Did:** (1) **Carryover open tabs** — bar dashboard now shows ALL open tabs (no date filter) + today's settled tabs filtered by `closed_at`. (2) **Close tab confirmation** — added `showConfirm` intermediate state before the payment screen. (3) **Bar regulars** — new `bar_regulars` table; `/bar/regulars` management page; autocomplete datalist on new tab form; `regular_id` FK on `pos_tabs`; "Save as regular" button on tab detail page with error display and rollback on orphaned insert. (4) **Inventory delete** — delete button added to admin inventory page. (5) **Artist delete UI** — `DeleteArtistButton` on artist list and edit pages; `deleteArtist` now nullifies `profiles.role` before deleting to prevent dangling portal sessions. (6) **`is_active` checkbox fix** — `formData.getAll("is_active").includes("true")` so checked state saves correctly. (7) **Server Component `onClick` fixes** — extracted `DeleteMenuItemButton` and `DeleteArtistButton` as client components (production crash fix). (8) **Bartender restock list** — "Needs Restocking" panel at bottom of `/bar/inventory` showing items below reorder threshold. (9) **Code review** — 8 findings fixed: `bar_regulars` RLS migration, `deleteArtist` session leak (nullify profile role), error surfacing in `SaveAsRegularButton` + `RegularsClient`, orphan rollback in `saveTabAsRegular`, threshold `<=` → `<`, `<a>` → `<Link>` in `OpenTabForm`, `type="button"` on edit/remove buttons, edit form auto-close on save. (10) **Custom item on tab** — `addCustomItem` server action + `CustomItemForm` client component; bartenders can enter an optional description and a JMD price to add any ad-hoc charge to an open tab; form auto-resets on success; placeholder shows `J$0.00`; page scrolls naturally (`pb-16`) — removed fixed height constraint.
**Touched:** `src/app/bar/page.tsx`, `src/app/bar/tabs/[id]/TabControls.tsx`, `src/app/bar/tabs/[id]/page.tsx`, `src/app/bar/tabs/[id]/actions.ts`, `src/app/bar/tabs/[id]/SaveAsRegularButton.tsx` (new), `src/app/bar/tabs/[id]/CustomItemForm.tsx` (new), `src/app/bar/tabs/new/OpenTabForm.tsx`, `src/app/bar/tabs/new/actions.ts`, `src/app/bar/regulars/` (new — page, actions, RegularsClient), `src/app/admin/bar/items/DeleteMenuItemButton.tsx` (new), `src/app/admin/bar/items/page.tsx`, `src/app/admin/bar/items/actions.ts`, `src/app/admin/bar/inventory/page.tsx`, `src/app/bar/inventory/page.tsx`, `src/app/admin/artists/DeleteArtistButton.tsx` (new), `src/app/admin/artists/actions.ts`, `src/app/admin/artists/page.tsx`, `src/app/admin/artists/[id]/edit/page.tsx`, `src/components/InkShell.tsx`, `src/types/supabase.ts`, `supabase/migrations/20260625000001_bar_regulars.sql` (new), `supabase/migrations/20260625000002_bar_regulars_rls.sql` (new)
...
**Did:** Four items. (1) **Bar staff UX** — `ResendInviteButton` client component; sends Supabase `magiclink` (not `recovery` — recovery fails after ban/unban cycles) via Resend with `redirectTo: /auth/callback`. `reactivateBartender` action (sets `ban_duration: "none"`). `set-password` page now role-routes after password set instead of always redirecting to `/admin` (bartenders → `/bar`, artists → `/portal`, etc.). (2) **Physical inventory count** — Migration `20260622175257_update_stock_quantities.sql` sets stock for all items from physical count; inserts three new items: Smirnoff, Rum Cream, Matterhorn (Single). Zero-stock items already disabled in `MenuGrid.tsx`. (3) **Mobile layout sweep** — Fixed layout across all admin and bar pages: collapsed `overflow-hidden`+inner `overflow-x-auto` table wrappers to single `overflow-x-auto` on the outer div (iOS Safari touch-scroll fix); applied to 9 table containers across 7 pages. Changed `grid-cols-3` → `grid-cols-1 sm:grid-cols-3` on admin stats grids (bar overview ×2, sales, member detail); kept `grid-cols-3` on bartender dashboard (intentional compact design). Added `flex-wrap justify-end` to campaigns page header. Added `overflow-x-auto + min-w-[480px]` to codes history table. Hidden Price/Sold Today columns on mobile in inventory. (4) **Code-review pass** — `/code-review` found 5 issues; fixed all: `items-end` → `items-start` on campaigns header (title was sliding down when buttons wrapped); 4 more pages with old overflow pattern fixed (admin/bar/members, admin/bar/tabs, admin/bar/items, bar/sessions); stock input restored to `w-16` (browser spinners clip at w-14); bar dashboard stats reverted to `grid-cols-3`.
**Touched:** `src/app/admin/bar/staff/ResendInviteButton.tsx` (new), `src/app/admin/bar/staff/actions.ts`, `src/app/admin/bar/staff/page.tsx`, `src/app/auth/set-password/page.tsx`, `supabase/migrations/20260622175257_update_stock_quantities.sql` (new, applied), `src/app/admin/bar/inventory/page.tsx`, `src/app/admin/bar/page.tsx`, `src/app/admin/bar/sales/page.tsx`, `src/app/admin/bar/members/[id]/page.tsx`, `src/app/admin/bar/members/page.tsx`, `src/app/admin/bar/tabs/page.tsx`, `src/app/admin/bar/items/page.tsx`, `src/app/admin/campaigns/page.tsx`, `src/app/admin/codes/page.tsx`, `src/app/bar/page.tsx`, `src/app/bar/sessions/page.tsx`
**Decided:** `type: "magiclink"` for resend invite (not `"recovery"` — recovery fails after ban/unban cycles with session_not_found). Bar dashboard stat cards stay `grid-cols-3` — compact by design for bartender phones.
**Blocked on:** First live bar session still pending. Flames Lounge gallery photos (placeholder). TikTok auto-posting (no Make.com module).
**Next:** First live bar session — promote an artist to bartender, open real tab, add items, close as cash, verify Jamaica timezone totals. Content entry: artists with photos, releases with cover art, first news post.
