  ...
  return (
    <div className="space-y-6 max-w-lg">
      <div>
        ...
        <p className="text-sm text-bone/60">{member.email}</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="border border-bone/15 rounded-xl p-4 text-center">
          ...
          <p className="text-xs text-bone/60 mt-1">Balance</p>
        </div>
        <div className="border border-bone/15 rounded-xl p-4 text-center">
          <p className={`text-lg font-semibold capitalize ${
            member.status === "active" ? "text-sage" : "text-red-400"
          }`}>{member.status}</p>
          <p className="text-xs text-bone/60 mt-1">Status</p>
        </div>
      </div>
      ...
      <section>
        ...
        {!sessions?.length ? (
          <p className="text-bone/50 text-sm">No sessions yet</p>
        ) : (
              ...
              <div key={s.id} className="flex items-center gap-3 border border-bone/10 rounded-lg px-3 py-2.5">
                <div className="flex-1 min-w-0">
                  <p className="text-bone text-sm">
                    ...
                    {s.station && <span className="text-bone/60 ml-2">· {s.station}</span>}
                  </p>
                </div>
