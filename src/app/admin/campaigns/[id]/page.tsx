...
const STATUS_STYLES: Record<string, string> = {
  ...
  approved:   "bg-forest/25 text-sage border border-forest/30",
...
export default async function CampaignDetailPage({
  ...
  return (
    <div className="space-y-8 max-w-5xl">
      {/* Header */}
      <div>
        <Link href="/admin/campaigns" className="inline-flex items-center gap-1.5 text-xs text-bone/52 hover:text-bone/70 transition-colors mb-4">
        ...
        <div className="flex items-start gap-4">
          <div className="flex-1 min-w-0">
            ...
            <p className="mt-1 text-xs text-bone/52">
      ...
      {isGenerating && (
        <div className="rounded-lg border border-forest/20 bg-forest/[0.04] p-5 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm text-sage font-medium">Generating content…</p>
            <p className="text-xs text-bone/60 mt-0.5">Claude is writing captions and generating images. This usually takes 1–3 minutes. Refresh to check progress.</p>
          </div>
          <a href={`/admin/campaigns/${id}`} className="ml-auto shrink-0 text-xs text-bone/50 hover:text-ochre transition-colors">
            Refresh ↻
          </a>
        </div>
      )}
      ...
      {(pieces ?? []).length > 0 ? (
        ...
        <div className="rounded-lg border border-bone/10 p-10 text-center text-bone/50 text-sm">
      ...
      <details className="group">
        <summary className="cursor-pointer text-xs text-bone/50 hover:text-bone/60 transition-colors list-none flex items-center gap-1.5">
        ...
        <pre className="mt-3 text-xs text-bone/60 leading-relaxed whitespace-pre-wrap font-sans bg-bone/5 rounded p-4 max-h-64 overflow-y-auto">
