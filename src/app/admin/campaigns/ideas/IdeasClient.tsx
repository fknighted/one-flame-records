...
const PILLAR_COLORS: Record<string, string> = {
  ...
  release_promotion: "bg-forest/15 text-forest border border-forest/20",
  ...
  label_news:        "bg-bone/5 text-bone/40 border border-bone/10",
};
    ...
    <div className={`rounded-lg border overflow-hidden transition-opacity ${idea.status === "expanded" ? "opacity-50" : "hover:border-bone/20"} border-bone/10`}>
      <div className="p-4 space-y-2.5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {idea.pillar && (
            <span className={`text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full ${PILLAR_COLORS[idea.pillar] ?? "bg-bone/10 text-bone/40"}`}>
              {pillarLabel(idea.pillar)}
            </span>
          )}
          <div className="flex gap-1 ml-auto">
            {(idea.suggested_platforms ?? []).map(p => (
              <span key={p} className="text-[10px] text-bone/30 font-mono">{PLATFORM_LABELS[p] ?? p}</span>
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 pt-2 border-t border-bone/8 flex-wrap">
          ...
          <form action={() => { startDismiss(() => dismissIdea(idea.id)); }} className="ml-auto">
            <button
              type="submit"
              disabled={dismissing}
              className="text-xs text-bone/20 hover:text-oxblood/60 transition-colors disabled:opacity-40"
            >
              Dismiss
            </button>
          </form>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-4">
        ...
        {draftCount > 0 && (
          <p className="text-xs text-bone/40">{draftCount} draft idea{draftCount !== 1 ? "s" : ""} ready to expand</p>
        )}
      </div>
      ...
      {visible.length === 0 ? (
        ...
        <div className="space-y-10">
          {PILLARS.map(pillar => {
            ...
            return (
              <div key={pillar.value}>
                <div className="mb-3">
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-bone/35">{pillar.label}</p>
                  <p className="text-[11px] text-bone/20 mt-0.5">{pillar.description}</p>
                </div>
          ...
          {(grouped["uncategorized"] ?? []).length > 0 && (
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-bone/25 mb-3">Other</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
