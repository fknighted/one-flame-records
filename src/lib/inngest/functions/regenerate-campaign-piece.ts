...
// image_needed is not stored — derive from content_type
function needsImage(contentType: string): boolean {
  return contentType === "image_post" || contentType === "story";
}
...
export const regenerateCampaignPiece = inngest.createFunction(
  ...
  async ({ event, step }) => {
    const { pieceId, campaignId } = event.data as { pieceId: string; campaignId: string };
...
Format: Hook (first 3 seconds) / Body / CTA. Plain text, no JSON.`,
        ...
        if (needsImage(piece.content_type) && process.env.OPENAI_API_KEY) {
          ...
          try {
            ...
            // Image failure doesn't fail the piece
          }
        }
